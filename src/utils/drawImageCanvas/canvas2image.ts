...
const Canvas2Image = (function () {
  ...
  function getDataURL(canvas: HTMLCanvasElement, type: any, width: any, height: any) {
  ...
  function fixType(type: { toLowerCase: () => string; match: (arg0: RegExp) => any[] }) {
    type = type.toLowerCase().replace(/jpg/i, 'jpeg') as any;
   ...
   * 按照规则生成图片响应头和响应体
   */
  const genBitmapImage = function (oData: { width: any; height: any; data: any }) {
    //
  ...
  const saveAsImage = function (
    canvas: string | HTMLElement | CanvasImageSource | null,
    width: any,
    height: any,
    ...
    if ($support.canvas && $support.dataURL) {
      ...
      type = fixType(type as any);
      if (/bmp/.test(type)) {
        const data = getImageData(scaleCanvas(canvas as any, width, height));
        const strData = genBitmapImage(data as any);
        // use new parameter: fileType
        saveFile(makeURI(strData, downloadMime), fileType as any, fileName);
      } else {
        const strData = getDataURL(canvas as any, type, width, height);
        // use new parameter: fileType
        saveFile(strData.replace(type, downloadMime), fileType as any, fileName);
      }
    }
  };
  ...
  return {
    ...
    saveAsPNG: function (canvas: any, width: any, height: any, fileName: any) {
      return saveAsImage(canvas, width, height, 'png', fileName);
    },
    saveAsJPEG: function (canvas: any, width: any, height: any, fileName: any) {
      return saveAsImage(canvas, width, height, 'jpeg', fileName);
    },
    saveAsGIF: function (canvas: any, width: any, height: any, fileName: any) {
      return saveAsImage(canvas, width, height, 'gif', fileName);
    },
    saveAsBMP: function (canvas: any, width: any, height: any, fileName: any) {
      return saveAsImage(canvas, width, height, 'bmp', fileName);
    },

    convertToImage: convertToImage,
    convertToPNG: function (canvas: any, width: any, height: any) {
      return convertToImage(canvas, width, height, 'png');
    },
    convertToJPEG: function (canvas: any, width: any, height: any) {
      return convertToImage(canvas, width, height, 'jpeg');
    },
    convertToGIF: function (canvas: any, width: any, height: any) {
      return convertToImage(canvas, width, height, 'gif');
    },
    convertToBMP: function (canvas: any, width: any, height: any) {
